...
from src.seedlabel.prompts.group_labeling import group_labeling_prompt

prompt_template = PromptTemplate(template=group_labeling_prompt)


def extract_labels_from_questions_group(state: RootState, llm, config=None):
    questions = state.get("questions") or []
    if not questions:
        raise ValueError("No question available")
    question: GroupQuestion = questions.pop()
